...
public class IntervalListIntersections {
    ...
    class Solution {
        public int[][] intervalIntersection(int[][] firstList, int[][] secondList) {
            ...
            while (pm < m && pn < n) {
                int startM = firstList[pm][0], endM = firstList[pm][1];
                int startN = secondList[pn][0], endN = secondList[pn][1];
                int maxS = Math.max(startM, startN);
                int minE = Math.min(endM, endN);
                if (minE >= maxS) {
                    ansList.add(new int[]{maxS, minE});
                }
                if (endM > endN) {
                    pn++;
                } else if (endM < endN) {
