...
public class MaximumProfitOfOperatingACentennialWheel {
    ...
    class Solution {
        public int minOperationsMaxProfit(int[] customers, int boardingCost, int runningCost) {
            int profit = 0, waitingPeople = 0;
            int ans = -1, max = 0;
            int index = 0;
            while (waitingPeople > 0 || index < customers.length) {
                if (index < customers.length) {
                    waitingPeople += customers[index];
                }
                profit += Math.min(4, waitingPeople) * boardingCost - runningCost;
                if (profit > max) {
                    max = profit;
                    ans = index + 1;
                }
                index++;
                waitingPeople = Math.max(0, waitingPeople - 4);
            }
            return ans;
        }
    }
