...
public class ShortestCompletingWord {
    ...
    class Solution {
        public String shortestCompletingWord(String licensePlate, String[] words) {
            Map<Character, Integer> map = wordCount(licensePlate);
            String result = null;
            for (int i = words.length - 1; i >= 0; i--) {
                boolean isCompleting = true;
                Map<Character, Integer> wordCountMap = wordCount(words[i]);
                for (Map.Entry<Character, Integer> entry : map.entrySet()) {
                    if (!wordCountMap.containsKey(entry.getKey())
                            || wordCountMap.get(entry.getKey()) < entry.getValue()) {
                        isCompleting = false;
                        break;
                    }
                }
