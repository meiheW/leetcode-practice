...
public class NumberOfOrdersInTheBacklog {
    ...
    class Solution {
        public int getNumberOfBacklogOrders(int[][] orders) {
            //int[] price amount
            //购买最大堆
            PriorityQueue<int[]> buyQueue = new PriorityQueue<>((o1, o2) -> Integer.compare(o2[0], o1[0]));
            //销售最小堆
            PriorityQueue<int[]> sellQueue = new PriorityQueue<>(Comparator.comparingInt(o -> o[0]));
            //price amount orderType
            for (int[] order : orders) {
                int price = order[0], account = order[1], type = order[2];
                if (type == 0) {//buy
                    while (account > 0 && !sellQueue.isEmpty() && sellQueue.peek()[0] <= price) {
                        int[] poll = sellQueue.poll();
                        if (poll[1] > account) {
                            sellQueue.add(new int[]{poll[0], poll[1] - account});
                        } else if (poll[1] < account) {
                            //buyQueue.add(new int[]{price, account - poll[1]});
                        }
                        account = Math.max(account - poll[1], 0);
                    }
                    if (account > 0) {
                        buyQueue.add(new int[]{price, account});
                    }
                } else {//sell
                    while (account > 0 && !buyQueue.isEmpty() && buyQueue.peek()[0] >= price) {
                        int[] poll = buyQueue.poll();
                        if (poll[1] > account) {
                            buyQueue.add(new int[]{poll[0], poll[1] - account});
                        } else if (poll[1] < account) {
                            //sellQueue.add(new int[]{price, account - poll[1]});
                        }
                        account = Math.max(account - poll[1], 0);
                    }
                    if (account > 0) {
                        sellQueue.add(new int[]{price, account});
                    }
                }
            }
            long sum = 0;
            while (!buyQueue.isEmpty()) {
                sum = (sum + buyQueue.poll()[1]) % 1_000_000_007;
            }
            while (!sellQueue.isEmpty()) {
                sum = (sum + sellQueue.poll()[1]) % 1_000_000_007;
            }
            return (int) sum;
        }
    }
