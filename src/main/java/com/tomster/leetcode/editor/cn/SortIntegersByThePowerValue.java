...
public class SortIntegersByThePowerValue {
    ...
    class Solution {
        public int getKth(int lo, int hi, int k) {
            int[] weights = new int[hi - lo + 1];
            for (int i = lo; i <= hi; i++) {
                weights[i - lo] = getWeight(i);
            }
            List<Integer> list = new ArrayList<>();
            for (int i = lo; i <= hi; i++) {
                list.add(i);
            }
            list.sort((o1, o2) -> weights[o1 - lo] == weights[o2 - lo] ?
                    Integer.compare(o1, o2) :
                    ((weights[o1 - lo] - weights[o2 - lo] > 0) ? 1 : -1));
            return list.get(k - 1);

        }

        private int getWeight(int v) {
            int step = 0;
            while (v != 1) {
                if (v % 2 == 0) {
                    v /= 2;
