    ...
    class Solution {
        public int[] exclusiveTime(int n, List<String> logs) {
            ...
            for (int i = 1; i < logs.size(); i++) {
                String[] segments = logs.get(i).split(":");
                int index = Integer.parseInt(segments[0]);
                String startOrEnd = segments[1];
                int curTime = Integer.parseInt(segments[2]);
                if ("start".equals(startOrEnd)) {
                    ...
                    result[stack.peek()] += curTime - preTime + 1;
                    stack.pop();
                    preTime = curTime + 1;
                }
