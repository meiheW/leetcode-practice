...
public class DesignAuthenticationManager {
    ...
    class AuthenticationManager {
        int timeToLive;
        Map<String, Integer> tokenExpiredTime;

        public AuthenticationManager(int timeToLive) {
            this.timeToLive = timeToLive;
            this.tokenExpiredTime = new HashMap<>();
        }

        public void generate(String tokenId, int currentTime) {
            tokenExpiredTime.put(tokenId, currentTime);
        }

        public void renew(String tokenId, int currentTime) {
            Integer preTime = tokenExpiredTime.get(tokenId);
            if (preTime == null) {
                return;
            }
            if (preTime + timeToLive <= currentTime) {
                tokenExpiredTime.remove(tokenId);
            } else {
                tokenExpiredTime.replace(tokenId, currentTime);
            }
        }

        public int countUnexpiredTokens(int currentTime) {
            /*Iterator<Map.Entry<String, Integer>> iterator = tokenExpiredTime.entrySet().iterator();
            ...
            tokenExpiredTime.entrySet().removeIf(entry -> entry.getValue() + timeToLive <= currentTime);
            return tokenExpiredTime.size();
        }
    }
